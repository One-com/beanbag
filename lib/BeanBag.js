...
_.extend(BeanBag.prototype, {
    ...
    getAgent: function () {
        if (!this._agent) {
            ...
            var Agent = require(URL.parse(this.url).protocol.replace(/:$/, '')).Agent;
    ...
    request: function (options, cb) {
        ...
        options.url = this.expandUrl(that.url, options);
        if (options.path) {
        ...
        options.url = addQueryStringToUrl(options.url, options.query);
        ...
        function makeRequest(callback) {
            ...
            return request(options).on('response', function (response) {
                seenResponse = true;
