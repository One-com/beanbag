 ...
 * config.maxConnections {Number} (optional) The maximum number of simultaneous TCP connections to each server. Defaults to 15
...
function BeanBag(config) {
    ...
    this.pool = {
        maxSockets: this.maxConnections || 15
    };
...
_.extend(BeanBag.prototype, {
    ...
    queryDesignDocument: function (options, cb) {
        ...
        if (!that.designDocument || !that.designDocument[viewName]) {
        ...
        if (options.temporary) {
            ...
            that.queryTemporaryView({view: that.designDocument[viewName], query: query, conditionalHeaders: conditionalHeaders}, cb);
        } else {
