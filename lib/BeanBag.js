        ...
        Object.keys(config).forEach(function (key) {
            if (key === 'cert' || key === 'key' || key === 'ca') {
                ...
                if (typeof value === 'string') {
                    value = fs.readFileSync(value.replace(/\{hostname\}/g, os.hostname()));
                }
                this[key] = value;
...
_.extend(BeanBag.prototype, {
    ...
    request: function (options, cb) {
        ...
        if (this.preprocessRequestOptions) {
            this.preprocessRequestOptions(requestOptions, passError(handleError, dispatchRequest));
        } else {
            dispatchRequest();
        }
        ...
        function dispatchRequest() {
            ...
            currentRequest.on('error', function (err) {
                ...
                that.emit('afterRequest', options, err, null, body, numRetries);
                if (numRetries > 0) {
                    numRetries -= 1;
                    dispatchRequest();
                } else {
                ...
                if (response.statusCode >= 400) {
                    return handleError(new httpErrors[response.statusCode]());
                        ...
                        if (matchFirstLine) {
                            ...
                            if (matchLastLine) {
                                ...
                                try {
                                    ...
                                    handleError(httpErrors.InternalServerError("Couldn't parse line: " + str));
                                }
                                eventEmitter.emit('row', row);
                            }
                        }
                    }).on('end', handleSuccess).on('error', handleError);
                } else if (cb) {
                        ...
                        if (responseBodyChunks.length > 0) {
                            ...
                            if (response.headers['content-type'] === 'application/json') {
                                try {
                                    ...
                                    return handleError(new httpErrors.BadGateway('Error parsing JSON response body'));
                                }
                            }
                        }
