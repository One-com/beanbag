...
_.extend(BeanBag.prototype, {
    ...
    request: function (options, cb) {
        ...
        function makeOnResponseRequest() {
            var seenResponse = false;

            request(options).on('response', function (response) {
                seenResponse = true;
                handleResponse(null, response);
            }).on('error', handleResponse); // Non-HTTP error
        }

        if (options.onResponse) {
            makeOnResponseRequest();
            ...
            request(options).on('response', function (response) {
                ...
                // Non-HTTP error, if the response event has fired, this will be picked up by `cb`
                if (!seenResponse) {
                    eventEmitter.emit('error', err);
                }
            });
    ...
    // private
    installDesignDocument: function (cb) {
        var that = this;
